...
//通过两个顶点找到连接它们的半边（Halfedge）[什么是Halfedge]
 ...
 */
//将网格上的SurfacePoint映射到另一个拓扑相同的网格上
//根据点类型（顶点、边、面）调整索引和参数，确保几何一致性
...
//从文件读取点云数据（顶点位置和法线），支持OBJ格式的v和vn标签
...
//读取曲线顶点（网格或点云），处理OBJ索引（1-based转0-based），支持曲线反转。
...
//将曲线和点源导出为OBJ文件。顶点位置写入v，连接关系写入l
...
//标准化SDF数据，使其适应色图范围。根据正负值的范围调整数值
...
//将函数或SDF数据导出为Wavefront OBJ文件或点云格式，支持表面网格、公共细分和点云
...
//创建本征三角剖分（Integer Coordinates），标记曲线对应的边以防止翻转
...
//初始化本征三角剖分的求解器（如Signed Heat Solver），处理曲线和点源的映射。
...
//将源几何（曲线和点）转换到本征三角剖分上，确保所有点约束在顶点
...
// ================ VISUALIZATION
//使用Polyscope可视化输入曲线和本征三角剖分的边（显示为橙色曲线网络）
...
//在本征三角剖分的公共细分网格上可视化数据（如SDF），支持标量场和等值线显示。
